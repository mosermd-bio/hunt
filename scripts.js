...
function generateSynopsis(geno) {
    let html = `<p class="font-medium">Your personalized genetic profile summary:</p><ul style="list-style:disc; padding-left:1.25rem; margin-top:1rem;">`;
    ...
    html += `</ul><p class="text-xs text-dim mt-lg">These are small-effect associations. Diet, antibiotics, and lifestyle have much larger impacts.</p>`;
    return html;
}
...
function buildTable(genotypes) {
    ...
    TARGET_SNPS.forEach(s => {
        const gt = genotypes[s.rsid] || '<span class="text-dim">Not genotyped</span>';
        let count = 0;
        if (typeof gt === "string" && gt.length === 2) count = (gt[0]===s.ea?1:0) + (gt[1]===s.ea?1:0);
        html += `<tr><td class="mono">${s.rsid}</td><td>${s.locus}</td><td class="font-medium">${gt}</td><td>${s.ea} <span class="text-dim">(ref ${s.ref})</span></td><td class="text-center font-medium">${count}</td><td class="text-sm">${s.effect}</td><td class="text-xs leading-relaxed">${s.interpretation}</td></tr>`;
    });
    html += `</tbody>`;
    return html;
}
...
document.getElementById('demoBtn').addEventListener('click', () => {
    ...
    tableEl.innerHTML = buildTable(genotypes);
